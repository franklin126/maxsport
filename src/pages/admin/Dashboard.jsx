...
export default function Dashboard() {
  ...
  const cargarVentasHoy = async () => {
    ...
    const { data, error } = await supabase.rpc('resumen_ventas', { p_desde: hoy.toISOString() });
    if (error) { console.error(error); return; }
    const fila = data && data[0];
    if (fila) {
      setVentasHoy({
        cantidad: Number(fila.total_transacciones) || 0,
        total: Number(fila.total_ingresos) || 0,
        unidades: Number(fila.total_unidades) || 0
      });
    }
  };
    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Link
            to="/admin/buscar-ticket"
            className="bg-gray-900 hover:bg-gray-800 border-2 border-cyan-600 rounded-xl p-8 transition group"
          >
            <div className="flex items-center gap-4">
              <div className="bg-cyan-600 p-4 rounded-lg group-hover:scale-110 transition">
