...
export default function AgregarProducto() {
  ...
  const [comprimiendo, setComprimiendo] = useState({ 1: false, 2: false, 3: false });
  ...
  const handleImagenChange = async (e, numeroImagen) => {
    ...
    setComprimiendo(prev => ({ ...prev, [numeroImagen]: true }));

    try {
      const comprimida = await comprimirImagen(file, { maxSizeMB: 0.4, maxWidthOrHeight: 1600 });
      const fileName = `${Date.now()}_${crypto.randomUUID().slice(0, 6)}_imagen${numeroImagen}.webp`;
      const { error } = await supabase.storage.from('productos').upload(`temp/${fileName}`, comprimida);
      if (error) { alert('Error al subir imagen'); return; }
      setImagenesTemp(prev => ({ ...prev, [`temp${numeroImagen}`]: `temp/${fileName}` }));
      setFormData(prev => ({ ...prev, [`imagen${numeroImagen}`]: comprimida }));
      ...
      setComprimiendo(prev => ({ ...prev, [numeroImagen]: false }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const marcaFinal = (formData.marca && formData.marca !== 'Sin marca') ? formData.marca : null;
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[1, 2, 3].map((num) => (
                <div key={num}>
                  ...
                  {comprimiendo[num] ? (
                    <div className="border-2 border-dashed border-blue-600 rounded-lg p-8 text-center h-48 flex flex-col items-center justify-center">
                      <RefreshCw className="animate-spin text-blue-400 mb-2" size={28} />
