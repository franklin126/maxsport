...
function TiendaPublica() {
  ...
  const construirQuery = (paginaActual) => {
    ...
    if (categoriaActual === '2x95') {
      query = query.eq('categoria', '2x95');
    } else if (categoriaActual === 'Artículos Deportivos') {
      query = query.eq('categoria', 'Artículos Deportivos');
      if (subcategoriaActual) query = query.eq('subcategoria', subcategoriaActual);
    } else if (categoriaActual === 'Ofertas') {
      query = query.eq('categoria', 'Ofertas');
    } else {
  ...
  useEffect(() => {
    if (categoriaActual === 'Artículos Deportivos' && !subcategoriaActual) {
      setProductos([]);
      setHayMas(false);
      setLoading(false);
      return;
  ...
  }, [categoriaActual, subcategoriaActual, searchTerm, tallaFiltro, marcaFiltro]);
